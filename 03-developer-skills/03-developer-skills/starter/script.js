...
// const fixedResult2 = calculateAverageScoreFixed(testScores);
...
function legacyForecastFunction(temperatures) {
  ...
  const cleaned = temperatures
    .map(Number)
    .filter(n => Number.isFinite(n));

  if (cleaned.length === 0) return "";

  const parts = cleaned.map((t, i) => {
    const day = i + 1;
    const label = day === 1 ? "day" : "days";
    return `${t}°C in ${day} ${label}`;
    });
...
console.log("Fixed function output:", legacyForecastFunction(testData));
